...
✓ LLM API — already configured (skip)
...
### Step 3: LLM Provider (Competitor Research)

**Required for:** AI-powered analysis of competitor content — summarizing transcripts, extracting hooks and angles, and scoring content against your ICP. Supports any OpenAI-compatible provider (OpenAI, OpenRouter, Together, etc.) or local Ollama.

**Auto-detect first:** Check if a key already exists in `.env`:
...
- If `LLM_API_KEY` or `OPENAI_API_KEY` is set and non-empty: show "Found LLM API key in .env (...{last 4 chars}). Testing..." and skip to verification.
- If not set: prompt the user:

Display:
```
────────────────────────────────────────
LLM PROVIDER — Competitor Research
────────────────────────────────────────

⚠️  RECOMMENDED — This powers competitor content analysis.

Without this, you can't automatically transcribe and analyze
competitor videos to reverse-engineer their hooks and angles.
Local Whisper + Ollama work as fallback but are significantly
slower and less accurate.

Works with any OpenAI-compatible provider. Cost varies:
- OpenAI: ~$0.006/min of audio, ~$0.15/M tokens for text
- OpenRouter: choose from many models, pay per token
- Ollama: free (local), but slower

Get your key from your preferred provider, then enter it below.
```

Wait for user input.

**Validation:**
- No strict format check (many providers use different key formats)
- If key is very short (< 10 chars): "That key looks very short. Most API keys are 20+ characters. Try again or 'skip'."

**If valid:**
- Write to `.env` file: `LLM_API_KEY={key}`
- Replace existing line if present
- If `OPENAI_API_KEY` also exists but `LLM_API_KEY` does not, leave the old `OPENAI_API_KEY` line — it serves as fallback

**Offer custom base URL:**
After the key is entered, ask:
```
This provider uses OpenAI-compatible endpoints. Is the base URL:
  1) Default OpenAI (api.openai.com/v1)
  2) Custom endpoint (e.g., OpenRouter, Together, etc.)
  3) I'll configure this later in .env

Choose (1-3):
```

If **2**: Prompt for `LLM_BASE_URL`, write to `.env`.

**If 'skip':**
- Note: "⚠️ LLM API skipped — competitor analysis will fall back to local Whisper + Ollama (slower, less accurate). Set LLM_API_KEY in .env anytime and re-run /viral:setup --reconfig"
...
The core system works best with YouTube + an LLM provider (OpenAI or any OpenAI-compatible endpoint).
...
### Step 2: LLM Provider API Test

Only if LLM key exists in `.env` (`LLM_API_KEY` or fallback `OPENAI_API_KEY`):

```bash
export $(grep -v '^#' .env | xargs) 2>/dev/null
LLM_URL="${LLM_BASE_URL:-https://api.openai.com/v1}"
LLM_KEY="${LLM_API_KEY:-$OPENAI_API_KEY}"
curl -s "$LLM_URL/models" -H "Authorization: Bearer $LLM_KEY" | head -c 500
```

Parse response:
- If response contains `"data"` → PASS
- If response contains `"error"` → FAIL, extract error message
- If no response → FAIL, "Network issue or invalid key. Check that LLM_BASE_URL is correct if using a non-OpenAI provider."
...
LLM API             [PASS/FAIL]  {error detail if fail}
...
- Add `"llm"` if LLM verification passed
...
- LLM/Transcription: Costs vary by provider (OpenAI ~$0.006/min audio, ~$0.15/M tokens text)
...
6. **Be helpful on errors** — if YouTube says "quotaExceeded", explain what that means. If the LLM provider says "invalid_api_key", say "Double-check your API key with your provider."
