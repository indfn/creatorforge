...
Uses configurable endpoints via env vars:
  OPENAI_REDDIT_BASE_URL — to derive model listing endpoint
  XAI_X_BASE_URL         — to derive model listing endpoint
"""
...
# Build model listing URLs from configured API base URLs (strip '/responses' if present)
_openai_base = os.getenv("OPENAI_REDDIT_BASE_URL", "https://api.openai.com/v1/responses")
# OpenAI models: go to /v1/models (strip /responses from the end, or just use default)
if _openai_base.endswith('/responses'):
    OPENAI_MODELS_URL = _openai_base.replace('/responses', '/models')
else:
    # Assume user's endpoint might not have the same layout; default to OpenAI
    OPENAI_MODELS_URL = "https://api.openai.com/v1/models"

OPENAI_FALLBACK_MODELS = ["gpt-5.2", "gpt-5.1", "gpt-5", "gpt-4.1", "gpt-4o"]

# xAI: same pattern
_xai_base = os.getenv("XAI_X_BASE_URL", "https://api.x.ai/v1/responses")
if _xai_base.endswith('/responses'):
    XAI_MODELS_URL = _xai_base.replace('/responses', '/models')
else:
